...
test('updating blog works', async() => {
  ...
  const check = await api
    ...
    .expect(200)
    .expect('Content-Type', /application\/json/)
  expect(check.body).toEqual({ id: '5a422a851b54a676234d17f7', title: 'React patterns', author: 'test author', url: 'https://reactpatterns.com/', likes: 7})
})

test('deleting blog returns code 204 and one blog is removed', async() => {
  const control = await api
    .get('/api/blogs')
    .expect('Content-Type', /application\/json/)
  await api
